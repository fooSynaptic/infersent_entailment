...
class BLSTMEncoder(nn.Module):
    ...
    def forward(self, sent_tuple):
        ...
        idx_sort = torch.from_numpy(idx_sort).cuda() if self.is_cuda() \
            else torch.from_numpy(idx_sort).cuda()
        sent = sent.index_select(1, Variable(idx_sort))
        #sent = sent.index_select(1, Variable(idx_sort.cpu()))

        # Handling padding in Recurrent Networks
        sent_packed = nn.utils.rnn.pack_padded_sequence(sent, sent_len)
        sent_output = self.enc_lstm(sent_packed)[0]  # seqlen x batch x 2*nhid
        #sent_output = self.enc_lstm(sent_packed)[0].flatten_paFrameters()
        ...
        #sent_output = sent_output.index_select(1, Variable(idx_unsort).cpu())
    ...
    def encode(self, sentences, bsize=64, tokenize=True, verbose=False):
        ...
        for stidx in range(0, len(sentences), bsize):
            batch = Variable(self.get_batch(
                        sentences[stidx:stidx + bsize].cuda()), volatile=True)
            if self.is_cuda():
                batch = batch.cuda()
            batch = self.forward(
                (batch, lengths[stidx:stidx + bsize])).data.cuda().numpy()
